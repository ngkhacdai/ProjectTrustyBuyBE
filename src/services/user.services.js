...
const findByEmail = async (user_email) =>{
    return await userSchema.findOne({user_email}).populate('user_avatar','thumbNail').select().lean()
}
const findById = async (userId) =>{
    return await userSchema.findById({_id:userId}).select().populate('user_avatar', 'thumbNail').lean();
}
...
const getImgProfiel = async (userId) =>{
    const user = await userSchema.findById(userId).populate('user_avatar', 'thumbNail');
